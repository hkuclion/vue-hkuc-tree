...
export default class HkucTreeInterface{
	...
	registerVm(id,$vm){
		...
		for (let stateField of Object.keys(this.setting.data.state)) {
			this.setNodeState(id,stateField,$vm.node[this.setting.data.state[stateField]]);
		}
	...
	setNodeVisible(id,visible){
		this.setNodeState(id, 'isHidden', !visible);

		let $vm = this.getVm(id);
		let parent_id = $vm.parentId;

		this.updateNodeLastVisibleChildIndex(parent_id);
	}
	...
	updateNodeLastVisibleChildIndex(id){
		let $vm = this.getVm(id);

		$vm.lastVisibleChildIndex = $vm.childNodeIds.indexOf($vm.childNodeIds.slice().reverse().find(childNodeId => this.getNodeState(childNodeId, 'isHidden') === false));
	}
	...
	setNodeState(id,stateName,value){
		...
		if(this.setting.data.state.hasOwnProperty(stateName)){
			$vm.node[this.setting.data.state[stateName]]=value;
		}
	...
	getSelectedNodes() {
		return this.selectedIds
			.filter(selectedId => selectedId !== 1)
			.sort((selectedId1, selectedId2) => this.getVm(selectedId1).level - this.getVm(selectedId2).level)
			.map(selectedId => this.getVm(selectedId).node)
	}
}